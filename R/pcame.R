...
#' @param pcaref A list with the elemements of a PCA model: 
#' * \code{m}: mean.
...
#' @return list with elements containing information about X in the PCA model: 
#' * \code{Xpreprocessed}: matrix \code{X} preprocessed.
#' * \code{Tscores}: score matrix with the projection of \code{X} on each one of the A PCs.
#' * \code{E}: error matrix with the par of \code{X} not explained by the PCA model.
#' * \code{SPE}: vector with the SPE for each observation of \code{X}.
#' * \code{T2}: vector with the T^_A for each observation of \code{X}.
#' * \code{T2matrix}: matrix with the contributions of each PC to the T^2_A for each observation 
#' of \code{X}.
#' * \code{Xrec}: matrix with the reconstructed part of \code{X}, i.e. the part of \code{X} 
